...
void Game::ringPut(uint8_t r, char i) {
...
void Game::readScroll(uint8_t r) {
  uint8_t ex = 0, i = 0, st, dx, dy;
  int fp;
  switch (ttab[1][r]) {
    case 0:       //protect
      st = equip(4, 1);
...
uint8_t Game::equip(uint8_t type, uint8_t n) {     //type=3(weapon),4(armor),7(ring), n=1 or 2(for ring)...rtab[equip(7,2)]
  uint8_t result = 0;
  for (int i = 0; i < 20; i++) {
    if (inv[i].ii / 16 == type) {
...
uint8_t Game::hasRing(uint8_t vari) {
  uint8_t result = 0;
...
int Game::checkHit(uint8_t dir, uint8_t str) {
  char dx = (dir - 2) * (dir % 2);
  char dy = (dir - 3) * ((dir - 1) % 2);
  uint8_t ex = 0, i = 0;
  uint8_t mon = 0;
  uint8_t x = hero.hx, y = hero.hy;
  do {
...
uint8_t Game::findPlace(uint8_t x, uint8_t y, uint8_t tm) {   //tm=0:thing, 1:monst
  uint8_t result = 0;
  uint8_t ex = 0, i = 0, r = random(8);
